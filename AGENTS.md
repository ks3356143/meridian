...
11. 能用 MCP 坚决用 MCP：浏览器截图、视觉验证等任务开工先检查可用的 MCP 工具；能用就用。Codex 应用内浏览器（cua_repl）在第三方 API key 认证下不可用（2026-09-25 实测报 unsupported Codex auth method: apikey），浏览器任务使用 Playwright MCP（本机 Edge channel）。仅当当前会话确实没有对应 MCP 工具时，才允许手写兑底脚本，且必须说明原因。Playwright 不用于替代截图视觉判断。
