...
CREATE TABLE requirement_events (
    ...
    action TEXT NOT NULL CHECK (action IN (
        'create', 'update', 'confirm', 'exclude', 'parse'
    )),
...
SELECT
    ...
    CASE WHEN action = 'restore' THEN 'confirm' ELSE action END,
    from_status, to_status,
    CASE WHEN action = 'restore' THEN '[restore] ' || detail ELSE detail END,
    operated_by, operated_at, created_at, updated_at
