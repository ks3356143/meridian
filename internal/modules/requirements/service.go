...
type RequirementIdentifierPreviewResponse struct {
...
func (s *Service) Workbench(ctx context.Context, projectCode string, sourceVersionID string) (WorkbenchResponse, error) {
	...
	if err := s.attachLastModificationMetadata(ctx, projectID, requirements, requirementResponses); err != nil {
		return WorkbenchResponse{}, err
	}
...
func (s *Service) attachLastModificationMetadata(
	ctx context.Context,
	projectID string,
	requirements []Requirement,
	responses []RequirementResponse,
	...
	if err := s.db.WithContext(ctx).Raw(`
		SELECT e.*
		FROM requirement_events AS e
		JOIN (
			SELECT requirement_id, MAX(rowid) AS rowid
			FROM requirement_events
			WHERE project_id = ? AND requirement_id IN ? AND action IN (?, ?)
			GROUP BY requirement_id
		) AS latest ON latest.requirement_id = e.requirement_id AND latest.rowid = e.rowid
	`, projectID, ids, "create", "update").Scan(&events).Error; err != nil {
		return fmt.Errorf("查询需求最近修改记录失败: %w", err)
	}
	...
	if len(operatorIDs) > 0 {
		var operators []users.User
		if err := s.db.WithContext(ctx).Where("id IN ?", operatorIDs).Find(&operators).Error; err != nil {
			return fmt.Errorf("查询需求最近修改人失败: %w", err)
		}
		for _, operator := range operators {
			name := operator.DisplayName
			if name == "" {
	...
	for _, event := range events {
		...
		responses[index].LastModifiedAction = event.Action
		responses[index].LastModifiedAt = event.OperatedAt.Format(time.RFC3339)
		responses[index].LastModifiedByName = operatorNames[event.OperatedBy]
	}
	return nil
}
...
func (s *Service) UpdateRequirement(ctx context.Context, input UpdateRequirementInput) (RequirementResponse, error) {
	...
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if updateErr := tx.Model(&Requirement{}).Where("id = ?", current.ID).Updates(map[string]any{
			...
			"identifier_auto_generated": externalID == "" ||
				(externalID == current.ExternalIdentifier && current.IdentifierAutoGenerated),
			"name":            name,
...
func createRequirementInTx(
	...
	identifierAutoGenerated := strings.TrimSpace(requestExternalID) == ""
	requirement := Requirement{
		...
		IdentifierAutoGenerated: identifierAutoGenerated,
...
func uniqueRequirementCode(tx *gorm.DB, sourceVersionID string, name string) (string, error) {
	code, _, err := generateUniqueRequirementCode(tx, sourceVersionID, name)
	return code, err
}

func generateUniqueRequirementCode(
	tx *gorm.DB,
	sourceVersionID string,
	name string,
) (string, bool, error) {
	...
	return "", false, fmt.Errorf("需求标识自动生成失败，请手工录入标识")
}

func (s *Service) PreviewRequirementIdentifier(
	ctx context.Context,
	projectCode string,
	sourceVersionID string,
	name string,
) (RequirementIdentifierPreviewResponse, error) {
	source, err := s.findSource(ctx, projectCode, sourceVersionID)
	if err != nil {
		return RequirementIdentifierPreviewResponse{}, err
	}

	trimmedName := strings.TrimSpace(name)
	if trimmedName == "" {
		return RequirementIdentifierPreviewResponse{}, fmt.Errorf("需求名称不能为空")
	}
	if len([]rune(trimmedName)) > 240 {
		return RequirementIdentifierPreviewResponse{}, fmt.Errorf("需求名称最多 240 个字符")
	}

	code, conflictAdjusted, err := generateUniqueRequirementCode(s.db, source.ID, trimmedName)
	if err != nil {
		return RequirementIdentifierPreviewResponse{}, err
	}
	return RequirementIdentifierPreviewResponse{
		Identifier:       code,
		ConflictAdjusted: conflictAdjusted,
	}, nil
}
...
func toRequirementResponse(requirement Requirement) RequirementResponse {
	return RequirementResponse{
		...
		IdentifierAutoGenerated: requirement.IdentifierAutoGenerated,
